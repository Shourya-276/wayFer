...
export const PhoneMockup: React.FC<PhoneMockupProps> = ({ screenId, className = '', glow = true }) => {
  ...
  return (
    <div className={`relative mx-auto select-none ${className}`}>
      {/* Outer ambient glow */}
      {glow && (
        <div className="absolute -inset-4 bg-gradient-to-tr from-white/15 via-white/5 to-transparent rounded-[52px] blur-2xl -z-10 opacity-70"></div>
      )}

      {/* Titanium / Obsidian Device Chassis */}
      <div className="relative w-[300px] h-[610px] sm:w-[320px] sm:h-[650px] bg-[#121212] rounded-[48px] p-3 shadow-phone-3d border-[3.5px] border-neutral-800 ring-1 ring-white/20">
        {/* Antennas & Side Button Accents */}
        <div className="absolute -left-[5.5px] top-24 w-1 h-8 bg-neutral-700 rounded-l-sm"></div>
        <div className="absolute -left-[5.5px] top-36 w-1 h-12 bg-neutral-700 rounded-l-sm"></div>
        <div className="absolute -right-[5.5px] top-32 w-1 h-14 bg-neutral-700 rounded-r-sm"></div>

        {/* Screen Bezel and Inner Display */}
        <div className="relative w-full h-full bg-black rounded-[38px] overflow-hidden border border-black flex flex-col justify-between">
          {!imageError ? (
            <img
              src={imageSrc}
            ...
            <>
              ...
              <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 flex items-center justify-between px-3 w-28 h-6 bg-black rounded-full shadow-md">
                ...
                <div className="w-2 h-2 rounded-full bg-neutral-900 border border-white/5"></div>
              </div>

              {/* Time & Battery Status Header */}
              <div className="relative z-20 flex items-center justify-between px-6 pt-3 pb-1 text-[10px] font-semibold text-neutral-300">
                <span>04:28</span>
                <div className="flex items-center gap-1.5 text-[9px]">
                  <span>5G</span>
                  <div className="w-4 h-2 rounded-sm border border-neutral-400 p-0.5 flex items-center">
              ...
              <div className="relative z-20 pb-2 pt-1 flex justify-center bg-black">
